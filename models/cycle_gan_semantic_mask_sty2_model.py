...
class CycleGANSemanticMaskSty2Model(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--lambda_f_s', type=float, default=1)
    ...
    def __init__(self, opt):
        ...
        if opt.loss_context:
            losses += ['wcontext_real_A','wcontext_real_B']
        if opt.loss_content:
 ...
 #           output_attn_B.append('output10_fake_B')
  #          output_attn_A.append('output10_fake_A')
        ...
        self.netG_A = networks.define_G(opt.input_nc, opt.output_nc,
                                        ...
                                        not opt.no_dropout, opt.G_spectral, opt.init_type, opt.init_gain, self.gpu_ids, decoder=False, wplus=opt.wplus, wskip=opt.wskip,img_size=opt.decoder_size,use_w_context=opt.use_w_context)
        ...
        self.netDecoderG_A = networks.define_decoder(init_type=opt.init_type, init_gain=opt.init_gain,gpu_ids=self.gpu_ids,size=opt.decoder_size,init_weight=not opt.no_init_weigth_dec_sty2)
        self.netDecoderG_B = networks.define_decoder(init_type=opt.init_type, init_gain=opt.init_gain,gpu_ids=self.gpu_ids,size=opt.decoder_size,init_weight=not opt.no_init_weigth_dec_sty2)
   ...
   #                                                kornia.color.AdjustBrightness(np.random.uniform(0.0,0.1)),
    ...
    def forward(self):
        ...
        temp_z_fake_B= self.netG_A(self.real_A)
        self.z_fake_B = temp_z_fake_B[0]

        self.output1_fake_B= temp_z_fake_B[1]
        self.output2_fake_B= temp_z_fake_B[2]
        self.output3_fake_B= temp_z_fake_B[3]
        self.output4_fake_B= temp_z_fake_B[4]
        self.output5_fake_B= temp_z_fake_B[5]
        self.output6_fake_B= temp_z_fake_B[6]
        self.output7_fake_B= temp_z_fake_B[7]
        self.output8_fake_B= temp_z_fake_B[8]
        self.output9_fake_B= temp_z_fake_B[9]
        self.n_fake_B= temp_z_fake_B[-1]
        ...
        if self.isTrain:
            ...
            if self.rec_noise:
                ...
                temp_z_rec_A= self.netG_B(self.fake_B_noisy1)
            else:
                temp_z_rec_A = self.netG_B(self.fake_B)

            self.z_rec_A = temp_z_rec_A[0]
            self.output1_rec_A= temp_z_rec_A[1]
            self.output2_rec_A= temp_z_rec_A[2]
            self.output3_rec_A= temp_z_rec_A[3]
            self.output4_rec_A= temp_z_rec_A[4]
            self.output5_rec_A= temp_z_rec_A[5]
            self.output6_rec_A= temp_z_rec_A[6]
            self.output7_rec_A= temp_z_rec_A[7]
            self.output8_rec_A= temp_z_rec_A[8]
            self.output9_rec_A= temp_z_rec_A[9]
            self.n_rec_A =  temp_z_rec_A[-1]
            if self.opt.use_w_context:
...
#            else:
 #               self.output10_rec_A= temp_z_rec_A[10]
            ...
            temp_z_fake_A= self.netG_B(self.real_B)
            
            self.z_fake_A = temp_z_fake_A[0]
            self.output1_fake_A= temp_z_fake_A[1]
            self.output2_fake_A= temp_z_fake_A[2]
            self.output3_fake_A= temp_z_fake_A[3]
            self.output4_fake_A= temp_z_fake_A[4]
            self.output5_fake_A= temp_z_fake_A[5]
            self.output6_fake_A= temp_z_fake_A[6]
            self.output7_fake_A= temp_z_fake_A[7]
            self.output8_fake_A= temp_z_fake_A[8]
            self.output9_fake_A= temp_z_fake_A[9]
            self.n_fake_A= temp_z_fake_A[-1]
            if self.opt.use_w_context:
                self.outputcontext_fake_A= temp_z_fake_A[10]
                #print(self.outputcontext_fake_A.abs().mean())
#            else:
 #               self.output10_fake_A= temp_z_fake_A[10]
            ...
            if self.rec_noise:
                ...
                temp_z_rec_B= self.netG_A(self.fake_A_noisy1)
            else:
                temp_z_rec_B = self.netG_A(self.fake_A)

            self.z_rec_B = temp_z_rec_B[0]
            self.output1_rec_B = temp_z_rec_B[1]
            self.output2_rec_B = temp_z_rec_B[2]
            self.output3_rec_B = temp_z_rec_B[3]
            self.output4_rec_B = temp_z_rec_B[4]
            self.output5_rec_B = temp_z_rec_B[5]
            self.output6_rec_B = temp_z_rec_B[6]
            self.output7_rec_B = temp_z_rec_B[7]
            self.output8_rec_B = temp_z_rec_B[8]
            self.output9_rec_B = temp_z_rec_B[9]
            self.n_rec_B =  temp_z_rec_B[-1]
            if self.opt.use_w_context:
...
#            else:
 #               self.output10_rec_B= temp_z_rec_B[10]
    ...
    def backward_D_B_mask_in(self):
        fake_A_mask_in = self.fake_A_pool.query(self.fake_A_mask_in)
    ...
    def backward_G(self):
        ...
        if lambda_idt > 0:
            ...
            temp_z_idt_A = self.netG_A(self.real_B)

            self.z_idt_A = temp_z_idt_A[0]
            self.output1_idt_A= temp_z_idt_A[1]
            self.output2_idt_A= temp_z_idt_A[2]
            self.output3_idt_A= temp_z_idt_A[3]
            self.output4_idt_A= temp_z_idt_A[4]
            self.output5_idt_A= temp_z_idt_A[5]
            self.output6_idt_A= temp_z_idt_A[6]
            self.output7_idt_A= temp_z_idt_A[7]
            self.output8_idt_A= temp_z_idt_A[8]
            self.output9_idt_A= temp_z_idt_A[9]
            self.n_idt_A= temp_z_idt_A[-1]
            if self.opt.use_w_context:
...
#            else:
 #               self.output10_idt_A= temp_z_idt_A[10]
            ...
            temp_z_idt_B = self.netG_B(self.real_A)

            self.z_idt_B = temp_z_idt_B[0]
            self.output1_idt_B= temp_z_idt_B[1]
            self.output2_idt_B= temp_z_idt_B[2]
            self.output3_idt_B= temp_z_idt_B[3]
            self.output4_idt_B= temp_z_idt_B[4]
            self.output5_idt_B= temp_z_idt_B[5]
            self.output6_idt_B= temp_z_idt_B[6]
            self.output7_idt_B= temp_z_idt_B[7]
            self.output8_idt_B= temp_z_idt_B[8]
            self.output9_idt_B= temp_z_idt_B[9]
            self.n_idt_B = temp_z_idt_B[-1]
            if self.opt.use_w_context:
...
#            else:
 #               self.output10_idt_B= temp_z_idt_B[10]
        ...
        # semantic loss AB
        self.loss_sem_AB = self.criterionf_s(self.pfB, self.input_A_label) *self.opt.lambda_f_s
        
        # semantic loss BA
        if hasattr(self, 'input_B_label'):
            self.loss_sem_BA = self.criterionf_s(self.pfA, self.input_B_label)*self.opt.lambda_f_s#.squeeze(1))
        else:
            self.loss_sem_BA = self.criterionf_s(self.pfA, self.gt_pred_B)*self.opt.lambda_f_s#.squeeze(1))
        ...
        self.loss_g_nonsaturating_A = self.opt.lambda_G*self.g_nonsaturating_loss(self.fake_pred_g_loss_A)
        
        #B
        self.fake_pred_g_loss_B = self.netDiscriminatorDecoderG_B(self.fake_B)
        self.loss_g_nonsaturating_B = self.opt.lambda_G*self.g_nonsaturating_loss(self.fake_pred_g_loss_B)
        
        self.loss_G += (self.loss_g_nonsaturating_A + self.loss_g_nonsaturating_B)

        if hasattr(self,'criterionWContext'):
            if self.opt.use_w_context:
                n = random.randint(0,2)
                cur = self.w_context_ref[n]
                #print(self.outputcontext_rec_A)
                ref_A = getattr(self,'outputcontext_'+cur+'_A').clone().detach()
                #print('outputcontext_'+cur+'_A')
                ref_B = getattr(self,'outputcontext_'+cur+'_B').clone().detach()
                #print(ref_A)
                #print(getattr(self,'outputcontext_'+self.w_context_ref[(n+1)%3]+'_A'))
                self.loss_wcontext_real_A=(self.criterionWContext(ref_A,getattr(self,'outputcontext_'+self.w_context_ref[(n+1)%3]+'_A'))+self.criterionWContext(ref_A,getattr(self,'outputcontext_'+self.w_context_ref[(n+2)%3]+'_A')) )* self.opt.lambda_w_context
                self.loss_wcontext_real_B=(self.criterionWContext(ref_B,getattr(self,'outputcontext_'+self.w_context_ref[(n+1)%3]+'_B'))+self.criterionWContext(ref_B,getattr(self,'outputcontext_'+self.w_context_ref[(n+2)%3]+'_B')) )* self.opt.lambda_w_context
            else:
                n = random.randint(0,2)
                cur = self.w_context_ref[n]
                ref_A = getattr(self,'output1_'+cur+'_A').clone().detach()
                ref_B = getattr(self,'output1_'+cur+'_B').clone().detach()
                self.loss_wcontext_real_A=(self.criterionWContext(ref_A,getattr(self,'output1_'+self.w_context_ref[(n+1)%3]+'_A'))+self.criterionWContext(ref_A,getattr(self,'output1_'+self.w_context_ref[(n+2)%3]+'_A')) )* self.opt.lambda_w_context
                self.loss_wcontext_real_B=(self.criterionWContext(ref_B,getattr(self,'output1_'+self.w_context_ref[(n+1)%3]+'_B'))+self.criterionWContext(ref_B,getattr(self,'output1_'+self.w_context_ref[(n+2)%3]+'_B')) )* self.opt.lambda_w_context
    ...
    def g_reg(self):
        #self.fake_A.requires_grad = True
        #print('self.fake_A.requires_grad',self.fake_A.requires_grad)
        
        temp_fake_A = self.fake_A_pool.query(self.fake_A)
        ...
        #print('2',self.fake_A.requires_grad)
        
        self.path_loss_A, self.mean_path_length_A, self.path_lengths_A = self.g_path_regularize(
                temp_fake_A, temp_latent_fake_A, self.mean_path_length_A
            )
        ...
        self.path_loss_B, self.mean_path_length_B, self.path_lengths_B = self.g_path_regularize(
            temp_fake_B, temp_latent_fake_B, self.mean_path_length_B
        )
        ...
        self.loss_weighted_path = self.loss_weighted_path_A + self.loss_weighted_path_B

        self.loss_weighted_path.backward()        
    ...
    def backward_discriminator_decoder(self):
        ...
        real_pred_B = self.netDiscriminatorDecoderG_B(self.real_B_pool.query(self.real_B))
    ...
    def d_reg(self):
        ...
        real_pred_A = self.netDiscriminatorDecoderG_A(cur_real_A)
        #r1_loss_A = self.d_r1_loss(real_pred_A_2, cur_real_A)
        
        self.loss_grad_pen_A = self.gradient_penalty(cur_real_A,real_pred_A)
        ...
        real_pred_B = self.netDiscriminatorDecoderG_B(cur_real_B)
        #r1_loss_B = self.d_r1_loss(real_pred_B_2, cur_real_B)
            
        self.loss_grad_pen_B = self.gradient_penalty(cur_real_B,real_pred_B)
        ...
        self.loss_grad_pen = self.loss_grad_pen_A + self.loss_grad_pen_B

        self.loss_grad_pen.backward()
    ...
    def optimize_parameters(self):
        ...
        if self.opt.path_regularize != 0.0 and self.opt.g_reg_every != 0 and self.niter % self.opt.g_reg_every == 0 :
            self.netDecoderG_A.zero_grad()
            self.netDecoderG_B.zero_grad()
            self.optimizer_G.zero_grad()  # set G_A and G_B's gradients to zero
        ...
        self.set_requires_grad([self.netDecoderG_A, self.netDecoderG_B], False)
        ...
        if self.opt.d_reg_every != 0 and self.niter % self.opt.d_reg_every == 0 :
            self.optimizer_D_Decoder.zero_grad()
            self.d_reg()
            self.backward_discriminator_decoder()
            self.optimizer_D_Decoder.step()
    ...
    def d_r1_loss(self,real_pred, real_img):
        grad_real, = torch.autograd.grad(
            outputs=real_pred.sum(), inputs=real_img#,allow_unused=True#, create_graph=True
        )
