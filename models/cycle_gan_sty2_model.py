...
class CycleGANSty2Model(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.netG_A = networks.define_G(opt.input_nc, opt.output_nc,
                                        ...
                                        not opt.no_dropout, opt.G_spectral, opt.init_type, opt.init_gain, self.gpu_ids, decoder=False, wplus=opt.wplus, wskip=opt.wskip,img_size=self.opt.crop_size,img_size_dec=opt.decoder_size)
        ...
        if self.isTrain:
            ...
            if self.rec_noise > 0.0:
                ...
                if self.opt.crop_size != self.opt.decoder_size:
                    self.fake_B_resized=nnf.interpolate(self.fake_B_noisy1, size=(self.opt.crop_size))
                    self.z_rec_A, self.n_rec_A = self.netG_B(self.fake_B_resized)
                else:
                    self.z_rec_A, self.n_rec_A = self.netG_B(self.fake_B_noisy1)
            else:
                if self.opt.crop_size != self.opt.decoder_size:
                    self.fake_B_resized=nnf.interpolate(self.fake_B, size=(self.opt.crop_size))
                    self.z_rec_A, self.n_rec_A = self.netG_B(self.fake_B_resized)
                else:
            ...
            if self.rec_noise > 0.0:
                ...
                if self.opt.crop_size != self.opt.decoder_size:
                    self.fake_A_resized=nnf.interpolate(self.fake_A_noisy1, size=(self.opt.crop_size))
                ...
                if self.opt.crop_size != self.opt.decoder_size:
                    self.fake_A_resized=nnf.interpolate(self.fake_A, size=(self.opt.crop_size))
                    self.z_rec_B, self.n_rec_B = self.netG_A(self.fake_A_resized)
                else:
    ...
    def backward_G(self):
        ...
        if lambda_idt > 0:
            ...
            if self.opt.crop_size != self.opt.decoder_size:
                self.loss_idt_A = self.criterionIdt(nnf.interpolate(self.idt_A, size=(self.opt.crop_size)), self.real_B) * lambda_B * lambda_idt
            else:
                self.loss_idt_A = self.criterionIdt(self.idt_A, self.real_B) * lambda_B * lambda_idt
            if self.percept_loss:
                if self.opt.crop_size != self.opt.decoder_size:
                    self.loss_idt_A += self.criterionIdt2(nnf.interpolate(self.idt_A, size=(self.opt.crop_size)), self.real_B) * lambda_B * lambda_idt
                else:
            ...
            if self.opt.crop_size != self.opt.decoder_size:
                self.loss_idt_B = self.criterionIdt(nnf.interpolate(self.idt_B, size=(self.opt.crop_size)), self.real_A) * lambda_A * lambda_idt
            else:
                self.loss_idt_B = self.criterionIdt(self.idt_B, self.real_A) * lambda_A * lambda_idt
            if self.percept_loss:
                if self.opt.crop_size != self.opt.decoder_size:
                    self.loss_idt_B += self.criterionIdt2(nnf.interpolate(self.idt_B, size=(self.opt.crop_size)), self.real_A) * lambda_A * lambda_idt
                else:
        ...
        # Forward cycle loss
        if self.opt.crop_size != self.opt.decoder_size:
            self.loss_cycle_A = self.criterionCycle(nnf.interpolate(self.rec_A, size=(self.opt.crop_size)), self.real_A) * lambda_A
        else:
            self.loss_cycle_A = self.criterionCycle(self.rec_A, self.real_A) * lambda_A
        if self.percept_loss:
            if self.opt.crop_size != self.opt.decoder_size:
                self.loss_cycle_A += self.criterionCycle2(nnf.interpolate(self.rec_A, size=(self.opt.crop_size)), self.real_A) * lambda_A
            else:
                self.loss_cycle_A += self.criterionCycle2(self.rec_A, self.real_A) * lambda_A
        # Backward cycle loss
        if self.opt.crop_size != self.opt.decoder_size:
            self.loss_cycle_B = self.criterionCycle(nnf.interpolate(self.rec_B, size=(self.opt.crop_size)), self.real_B) * lambda_B
        else:
            self.loss_cycle_B = self.criterionCycle(self.rec_B, self.real_B) * lambda_B
        if self.percept_loss:
            if self.opt.crop_size != self.opt.decoder_size:
                self.loss_cycle_B += self.criterionCycle2(nnf.interpolate(self.rec_B, size=(self.opt.crop_size)), self.real_B) * lambda_B
            else:
        ...
        #A
        if self.opt.crop_size != self.opt.decoder_size:
            self.fake_pred_g_loss_A = self.netDiscriminatorDecoderG_A(nnf.interpolate(self.fake_A, size=(self.opt.crop_size)))
        else:
        ...
        #B
        if self.opt.crop_size != self.opt.decoder_size:
            self.fake_pred_g_loss_B = self.netDiscriminatorDecoderG_B(nnf.interpolate(self.fake_B, size=(self.opt.crop_size)))
        else:
    ...
    def backward_discriminator_decoder(self):
        ...
        if self.opt.crop_size != self.opt.decoder_size:
            fake_pred_A = self.netDiscriminatorDecoderG_A(nnf.interpolate(self.fake_A_pool.query(self.fake_A), size=(self.opt.crop_size)))
        else:
        ...
        if self.opt.crop_size != self.opt.decoder_size:
            fake_pred_B = self.netDiscriminatorDecoderG_B(nnf.interpolate(self.fake_B_pool.query(self.fake_B), size=(self.opt.crop_size)))
        else:
